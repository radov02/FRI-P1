...
public class Blok {
    ...
    private void najdiNacrtBloka(Stanovanje trenutno, int x, int y){
        ...
        najdiObrnjenNacrtBloka(trenutno, trenutno, x, y, new Stanovanje[1000], 0);

        Stanovanje[][] nacrt = new Stanovanje[nacrtBloka.length][nacrtBloka[0].length];

        for(int i = 0; i < nacrtBloka.length; i++){
            nacrt[nacrt.length - 1 - i] = nacrtBloka[i];
        }
    ...
    private Stanovanje najdiObrnjenNacrtBloka(Stanovanje trenutno, Stanovanje prejsnje, int x, int y, Stanovanje[] zePreverjena, int indexZePreverjena){
        ...
        for(int i = 0; i < 4; i++){
            if(trenutno.getSosednjaStanovanja()[i] != null && !this.jeZeBiloPreverjeno(trenutno.getSosednjaStanovanja()[i], zePreverjena)){
                ...
                this.nb[novaY][novaX] = this.nacrtBloka[novaY][novaX].crka;
